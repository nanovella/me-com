...
function LOGINopenSpotify() {
    ...
    '&redirect_uri=https%3A%2F%2Fprojectredland.com%2Fcallback&scope=user-read-private%20user-modify-playback-state&state=34fFs29kd09', 
    {
        method: 'GET'        
    }).then((response) => {
...
function buildSpotify() {
    ...
    let mainControls = document.createElement("button");
    mainControls.className = "mainControls"
    mainControls.onclick = () => {playMusic(play)};

    let play = document.createElement("button");
    play.className = "playbutton pause"
    play.onclick = () => {playMusic(play)};

    let foward = document.createElement("button");
    foward.className = "fowardButton"
    foward.onclick = () => {playMusic(play)};
    let back = document.createElement("button");
    back.className = "backButton"
    back.onclick = () => {playMusic(play)};


    document.getElementById("mainControls").appendChild(play);
    document.getElementById("mainControls").appendChild(foward);
    document.getElementById("mainControls").appendChild(back);
    document.getElementById("audioPanel").appendChild(mainControls);
}

console.log(document.getElementById("clockItem").Properties);
document.getElementById("clockItem").Properties.transform = {
  ...
  document.getElementById("clockItem").implement({
